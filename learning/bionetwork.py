...
class model(torch.nn.Module):
    def __init__(self, networkList, nodeNames, modeOfAction, inputAmplitude, projectionFactor, inName, outName,
                 bionetParams, activationFunction='MML', valType=torch.double):
...
def spectralLoss(signalingModel, YhatFull, weights, expFactor=20, lb=0.5):
    ...
    activationFactor = signalingModel.oneStepDeltaActivationFactor(YhatFull[randomIndex, :],
                                                                   bionetParams['leak']).detach()
...
class bionet(nn.Module):
    ...
    def forward(self, x):
        return bionetworkFunction.apply(x, self.weights, self.bias, self.A, self.networkList, self.param,
                                        self.activation, self.delta)
...
def loadNetwork(filename, banList=[]):
    ...
    ### Added to get rid of sink nodes    
    #unique_sources = numpy.unique(list(net["source"]))
    #unique_targets = numpy.unique(list(net["target"]))
    
    sources = list(net["source"])
    targets = list(net["target"])
    # Added if in case you do not know sign
    modeOfAction = 0.1 * numpy.ones(len(sources))
    if ("stimulation" in net.columns) and ("inhibition" in net.columns):
    ...
    networkList, nodeNames, weights = makeNetworkList(sources, targets,
                                                      modeOfAction)  # 0 == Target 1 == Source due to numpy sparse matrix structure
...
def makeNetworkList(sources, targets, weights):
    ...
    networkList = numpy.array(
        (networkList[1], networkList[0]))  # 0 == Target 1 == Source due to numpy sparse matrix structure
...
class bionetworkAutoGrad(nn.Module):
    def __init__(self, networkList, size, reps=150):
        ...
        self.A = torch.sparse.FloatTensor(torch.from_numpy(networkList).long(), weights,
                                          torch.Size([size, size])).double()
